...
!function(){"use strict";function e(e,t,o,r){o.baseUrl=r.baseUrl,o.loginUrl="/auth",e.state("app",{url:"","abstract":!0,template:'<div id="base" ui-view></div>',controller:"BaseCtrl as Base"}).state("app.master",{url:"",templateUrl:"/cache/layouts/master.html","abstract":!0}).state("app.simple",{url:"",templateUrl:"/cache/layouts/simple.html","abstract":!0}).state("app.master.hangman",{url:"/hangman","abstract":!0,controller:"HangmanCtrl as Hangman",template:"<div ui-view></div>"}).state("app.master.hangman.index",{url:"/",title:"Hangman",controller:"HangmanIndexCtrl as Hangman",templateUrl:"/cache/games/hangman/index.html"}).state("app.master.hangman.create",{url:"/create",title:"Hangman | create",controller:"HangmanCreateCtrl as Hangman",templateUrl:"/cache/games/hangman/create.html"}).state("app.master.hangman.lobby",{url:"/:id/lobby",title:"Hangman | Lobby",controller:"HangmanLobbyCtrl as Hangman",templateUrl:"/cache/games/hangman/lobby.html"}).state("app.master.profile",{url:"/profile",title:"Profile",controller:"ProfileCtrl as Profile",templateUrl:"/cache/profile/index.html"}).state("app.master.profile-edit",{url:"/profile/edit",title:"Profile | Edit",controller:"ProfileCtrl as Profile",templateUrl:"/cache/profile/edit.html"}).state("app.simple.register",{url:"/register",title:"Register",controller:"RegisterCtrl as Register",templateUrl:"/cache/sessions/register.html"}),t.otherwise("/profile")}angular.module("orange.config",["orange.constants"]).config(e),e.$inject=["$stateProvider","$urlRouterProvider","$authProvider","Config"]}(),function(){"use strict";angular.module("orange.constants",[]).constant("Config",{baseUrl:"http://localhost:4200/",siteName:"Orange Gaming"})}(),function(){"use strict";angular.module("orange",["btford.socket-io","ngAnimate","satellizer","http-auth-interceptor","ui.router","orange.templates","orange.config","orange.factories","orange.services","orange.controllers","orange.run"])}(),function(){"use strict";function e(e,t,o){$(document).ready(function(){$(".dropdown-button").dropdown()}),e.$on("$stateChangeSuccess",function(){e.title=t.current.title,o.clear()})}angular.module("orange.run",[]).run(e),e.$inject=["$rootScope","$state","ErrorsFactory"]}(),function(){"use strict";function e(e,t,o,r,n,a,s){function c(){t.session=n.session.session,t.config=a,t.errorCollection=s.errorCollection,u.data={forms:{login:{username:"",password:""}}},console.log("Loading base controller"),n.profile()}function i(){$(function(){"app.simple.register"!=o.current.name&&$("#login").openModal()})}function l(){$(function(){$("#login").closeModal()})}var u=this;c(),e.$on("event:auth-loginRequired",function(e,t){n.logout(),i()}),e.$on("event:auth-loginConfirmed",function(e,t){l()}),t.register=function(){l(),o.go("app.simple.register")},t.login=function(){n.login(u.data.forms.login).then(function(e){u.data.forms.login={},r.loginConfirmed(),l()},function(e){console.log(e)})},t.logout=function(){e.$broadcast("event:auth-loginRequired")}}angular.module("orange.controller.base",[]).controller("BaseCtrl",e),e.$inject=["$rootScope","$scope","$state","authService","SessionsFactory","Config","ErrorsFactory"]}(),function(){"use strict";function e(e,t,o,r,n){function a(){console.log("Loading the base hangman controller"),e.socket=n.hangman(),e.games=[],e.socket.on("connect",function(){e.socket.emit("authenticate",{token:localStorage.getItem("satellizer_token")})}),e.socket.on("unauthorized",function(){t.$broadcast("event:auth-loginRequired")}),e.socket.on("games:updated",function(t){console.log("Received a games:updated event"),e.games=t,e.$broadcast("games:updated")}),e.socket.on("game:error",function(e){console.log(e)}),e.socket.on("error",function(e){"UnauthorizedError"==e.type||"invalid_token"==e.code?console.log("User's token has expired"):console.log(e)})}a()}angular.module("orange.controller.hangman.base",[]).controller("HangmanCtrl",e),e.$inject=["$scope","$rootScope","$state","$timeout","SocketFactory"]}(),function(){"use strict";angular.module("orange.controllers",["orange.controller.base","orange.controller.hangman.base","orange.controllers.hangman","orange.controller.profile","orange.controller.register"])}(),function(){"use strict";function e(e,t,o){function r(){console.log("loaded the profile controller"),n.data={forms:{edit:{}}}}var n=this;r(),n.update=function(){o.update(e.session.current_user).then(function(){t.go("app.master.profile")},function(){})}}angular.module("orange.controller.profile",[]).controller("ProfileCtrl",e),e.$inject=["$scope","$state","UsersFactory"]}(),function(){"use strict";function e(e,t,o){function r(){n.data={forms:{register:{}}}}var n=this;r(),n.register=function(){o.register(n.data.forms.register).then(function(){n.data.forms.register={},t.go("app.master.profile")},function(){})}}angular.module("orange.controller.register",[]).controller("RegisterCtrl",e),e.$inject=["$scope","$state","SessionsFactory"]}(),function(){"use strict";function e(e){function t(t){r.errors=e.format(t),console.log(r.errors)}function o(){r.errors=null}var r={errors:null},n={errorCollection:r,registration:t,clear:o};return n}angular.module("orange.factory.errors",[]).factory("ErrorsFactory",e),e.$inject=["FormErrorsService"]}(),function(){"use strict";function e(e,t,o){function r(){var r=t.defer();return e.get(o.baseUrl+"api/games/hangman").success(function(e){r.resolve(e.data)}).error(function(e){console.log(e),r.reject()}),r.promise}var n={all:r};return n}angular.module("orange.factory.games.hangman",[]).factory("HangmanFactory",e),e.$inject=["$http","$q","Config"]}(),function(){"use strict";angular.module("orange.factories",["orange.factory.socket","orange.factory.sessions","orange.factory.errors","orange.factory.users","orange.factory.games.hangman"])}(),function(){"use strict";function e(e,t,o,r,n,a,s,c,i){function l(e){c.create(d,e)}function u(e){var t=r.defer(),o={ignoreAuthModule:!0};return n.login(e,o).then(function(e){c.create(d,e.data.user),t.resolve()})["catch"](function(e){t.reject(e)}),t.promise}function g(){n.logout(),c.destroy(d)}function m(){var n=localStorage.getItem("current_user"),s=r.defer();return"null"===n||null===n||void 0===n||"undefined"===n?(t(function(){o.$broadcast("event:auth-loginRequired")},1),s.reject()):(n=JSON.parse(n),e.get(a.baseUrl+"api/users/"+n.id).success(function(e){c.create(d,e.data),s.resolve()}).error(function(e){s.reject(e)})),s.promise}function f(t){var o=r.defer();return e({method:"POST",url:a.baseUrl+"sessions/sign-up",data:t}).success(function(e){c.create(d,e.user,e.token),o.resolve()}).error(function(e){i.registration(e),o.reject()}),o.promise}var d={session:{current_user:null,logged_in:!1}},p={login:u,logout:g,profile:m,register:f,session:d,create:l};return p}angular.module("orange.factory.sessions",[]).factory("SessionsFactory",e),e.$inject=["$http","$timeout","$rootScope","$q","$auth","Config","JWTService","SessionsService","ErrorsFactory"]}(),function(){"use strict";function e(e){function t(){return e.create("hangman")}function o(){return e.create("blackjack")}var r={hangman:t,blackjack:o};return r}angular.module("orange.factory.socket",[]).factory("SocketFactory",e),e.$inject=["SocketService"]}(),function(){"use strict";function e(e,t,o,r){function n(n){var a=t.defer();return e({method:"PUT",url:o.baseUrl+"api/users/"+n.id,data:n}).success(function(e){r.create(e.data),a.resolve()}).error(function(e){console.log(e),a.reject()}),a.promise}var a={update:n};return a}angular.module("orange.factory.users",[]).factory("UsersFactory",e),e.$inject=["$http","$q","Config","SessionsFactory"]}(),function(){"use strict";function e(){function e(e,t){var o=0;for(o;o<t.length;o++)return t[o]===e?!0:!1}var t=this;t.format=function(t){var o={};if(t.length&&t[0].hasOwnProperty("param")&&t[0].hasOwnProperty("msg")){var r=0;for(r;r<t.length;r++){var n=o[t[r].param];if(n)e(t[r].msg,n)||o[t[r].param].push(t[r].msg);else{var a=t[r].param;"name.first"==a?a="first_name":"name.last"==a&&(a="last_name"),o[a]=[t[r].msg]}}}return o}}angular.module("orange.service.form_errors",[]).service("FormErrorsService",e),e.$inject=[]}(),function(){"use strict";function e(e){var t=this;t.parse=function(t){var o=t.split(".")[1],r=o.replace("-","+").replace("_","/");return JSON.parse(e.atob(r))},t.save=function(t){e.localStorage._satellizer_token=t},t.get=function(){return e.localStorage.satellizer_token}}angular.module("orange.service.jwt",[]).service("JWTService",e),e.$inject=["$window"]}(),function(){"use strict";angular.module("orange.services",["orange.service.jwt","orange.service.sessions","orange.service.form_errors","orange.service.socket"])}(),function(){"use strict";function e(e){var t=this;t.create=function(t,o,r){console.log(o),e.localStorage.current_user=JSON.stringify(o),r&&""!==r&&null!==r&&"null"!==r&&($http.defaults.headers.common.Authorization="Bearer "+res.token,e.localStorage.satellizer_token=r,e.localStorage._satellizer_token=r),t.session.current_user=o,t.session.logged_in=!0},t.destroy=function(t){e.localStorage.removeItem("_satellizer_token"),e.localStorage.removeItem("satellizer_token"),e.localStorage.removeItem("current_user"),t.session.current_user={},t.session.logged_in=!1}}angular.module("orange.service.sessions",[]).service("SessionsService",e),e.$inject=["$window"]}(),function(){"use strict";function e(e){this.create=function(t){var o=io.connect("http://localhost:4200/"+t);return e({ioSocket:o})}}angular.module("orange.service.socket",[]).service("SocketService",e),e.$inject=["socketFactory"]}(),function(){"use strict";function e(e,t,o){function r(){n.data={forms:{create:{status:"created",players:[]}}},o(function(){n.data.forms.create.owner=e.session.current_user},500)}var n=this;r(),e.socket.on("game:created",function(e){console.log(e),t.go("app.master.hangman.lobby",{id:e.id})}),n.create=function(){e.socket.emit("create",n.data.forms.create)}}angular.module("orange.controller.hangman.create",[]).controller("HangmanCreateCtrl",e),e.$inject=["$scope","$state","$timeout"]}(),function(){"use strict";function e(e){function t(){o.data={}}var o=this;t()}angular.module("orange.controller.hangman.game",[]).controller("HangmanGameCtrl",e),e.$inject=["$scope"]}(),function(){"use strict";function e(e,t){function o(){r.data={games:[]},e.socket.emit("games:request")}var r=this;o(),e.$on("games:updated",function(){console.log("games were updated"),r.data.games=e.games}),e.socket.on("game:leave",function(e){console.log("%s just left the room.",e.full_name)}),e.socket.on("game:ended",function(){console.log("Game just closed")})}angular.module("orange.controller.hangman.index",[]).controller("HangmanIndexCtrl",e),e.$inject=["$scope","HangmanFactory"]}(),function(){"use strict";function e(e,t){function o(){console.log("Loading the hangman lobby controller"),r.data={game:null},console.log("trying to join: %s",t.id),e.socket.emit("join",e.session.current_user,t.id)}var r=this;o(),r.ready=function(){},r.notReady=function(){},e.socket.on("game:joined",function(e){console.log("%s just joined the game!",e.player.full_name),r.data.game=e.game}),e.socket.on("game:leave",function(e){console.log("%s just left the game!",e.player.full_name),r.data.game=e.game}),e.$on("$destroy",function(){console.log("%s is leaving the lobby",e.session.current_user.id),e.socket.emit("leave",e.session.current_user,r.data.game.id)})}angular.module("orange.controller.hangman.lobby",[]).controller("HangmanLobbyCtrl",e),e.$inject=["$scope","$stateParams"]}(),function(){"use strict";angular.module("orange.controllers.hangman",["orange.controller.hangman.index","orange.controller.hangman.create","orange.controller.hangman.lobby"])}();
angular.module("orange.templates",[]).run(["$templateCache",function(e){e.put("/cache/chat.html",'<ul id=messages></ul><div id=typing></div><div class=row><form class="col s12"><div class=row><div class="input-field col s12"><input id=m type=text class=validate> <label for=first_name>Message</label></div></div><button type=submit class="btn waves-effect">Send</button></form></div>'),e.put("/cache/games/index.html","games index"),e.put("/cache/includes/_nav.html",'<div class=navbar-fixed><nav><div class=nav-wrapper><a href=#! class=brand-logo>Logo</a><ul class="right hide-on-med-and-down"><li><a href=sass.html>Sass</a></li><li><a href=badges.html>Components</a></li></ul></div></nav></div>'),e.put("/cache/layouts/master.html",'<div class=navbar-fixed><ul id=dropdown1 class=dropdown-content><li><a ui-sref=app.master.profile>profile</a></li><li class=divider></li><li><a href="" ng-click=logout()>Logout</a></li></ul><nav><div class=nav-wrapper><a href=# class=brand-logo>{{config.siteName}}</a><ul id=nav-mobile class="right hide-on-med-and-down"><li ng-if=session.logged_in><a ui-sref=app.master.hangman.index>Hangman</a></li><li><a class=dropdown-button href=#! data-activates=dropdown1>{{session.current_user.full_name}}<i class="material-icons right"></i></a></li></ul></div></nav></div><div class="container full-width"><div class=row><div class="col s12" id=content ui-view=""></div></div></div><div id=login class=modal><div class=modal-content><h4>Login</h4><div class=row><form class="col s12" ng-submit=login()><div class=row><div class="input-field col s12"><input ng-model=Base.data.forms.login.username id=username type=text class=validate> <label for=first_name>Username</label></div><div class="input-field col s12"><input id=last_name type=password ng-model=Base.data.forms.login.password class=validate> <label for=last_name>Password</label></div></div><button class="btn waves-effect" type=submit>Login</button></form></div><div class=row><div class="col s12"><p>Don\'t have an account? That\'s alright, register here :)</p><button class="btn pink waves-effect" ng-click=register()>Register</button></div></div></div></div>'),e.put("/cache/layouts/simple.html",'<div class=container><div class=row><div class="col s12" ui-view=""></div></div></div>'),e.put("/cache/profile/edit.html",'<div class="card-panel light-blue lighten-3"><div class=row><form class="col s12" ng-submit=Profile.update()><div class=row><div class="input-field col s6"><input ng-model=session.current_user.name.first id=first_name type=text class=validate> <label class=white-text for=first_name>First Name</label><div ng-if=errorCollection.errors.first_name class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.first_name"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div><div class="input-field col s6"><input ng-model=session.current_user.name.last id=last_name type=text class=validate> <label class=white-text for=last_name>Last Name</label><div ng-if=errorCollection.errors.last_name class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.last_name"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><div class=row><div class="input-field col s12"><input ng-model=session.current_user.username id=username type=text class=validate> <label class=white-text for=username>Username</label><div ng-if=errorCollection.errors.username class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.username"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><div class=row><div class="input-field col s12"><input ng-model=session.current_user.email id=email type=email class=validate> <label class=white-text for=email>Email</label><div ng-if=errorCollection.errors.email class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.email"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><div class=row><div class="input-field col s12"><input ng-model=session.current_user.age id=age type=tel class=validate> <label class=white-text for=age>Age</label><div ng-if=errorCollection.errors.age class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.age"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><div class=row><div class="input-field col s12"><input ng-model=session.current_user.password id=password type=password class=validate> <label class=white-text for=password>Password</label><div ng-if=errorCollection.errors.password class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.password"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><button class="btn pink waves-effect" type=submit>Update</button></form></div></div>'),e.put("/cache/profile/index.html",'<div class=row><div class="col s8 offset-s2"><div class="card-panel light-blue lighten-3"><div class=section><span class=white-text><p><b>Name</b>: {{session.current_user.full_name}}</p><p><b>Username</b>: {{session.current_user.username}}</p><p><b>Email</b>: {{session.current_user.email}}</p><p><b>Age</b>: {{session.current_user.age}}</p></span></div><div class=divider></div><div class=section><button class="btn pink lighten-2 waves-effect" ui-sref=app.master.profile-edit>Edit</button></div></div></div></div>'),e.put("/cache/sessions/register.html",'<div class="card-panel light-blue lighten-3"><h2>Register</h2><div class=row><form class="col s12" ng-submit=Register.register()><div class=row><div class="input-field col s6"><input ng-model=Register.data.forms.register.name.first id=first_name type=text class=validate> <label class=white-text for=first_name>First Name</label><div ng-if=errorCollection.errors.first_name class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.first_name"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div><div class="input-field col s6"><input ng-model=Register.data.forms.register.name.last id=last_name type=text class=validate> <label class=white-text for=last_name>Last Name</label><div ng-if=errorCollection.errors.last_name class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.last_name"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><div class=row><div class="input-field col s12"><input ng-model=Register.data.forms.register.username id=username type=text class=validate> <label class=white-text for=username>Username</label><div ng-if=errorCollection.errors.username class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.username"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><div class=row><div class="input-field col s12"><input ng-model=Register.data.forms.register.password id=password type=password class=validate> <label class=white-text for=password>Password</label><div ng-if=errorCollection.errors.password class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.password"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><div class=row><div class="input-field col s12"><input ng-model=Register.data.forms.register.email id=email type=email class=validate> <label class=white-text for=email>Email</label><div ng-if=errorCollection.errors.email class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.email"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><button type=submit class="btn blue waves-effect">Get Started</button></form></div></div>'),e.put("/cache/games/hangman/create.html",'<div class="card-panel light-blue lighten-3"><div class=row><form class="col s12" ng-submit=Hangman.create()><div class=row><div class="input-field col s6"><input ng-model=Hangman.data.forms.create.room id=room type=text class=validate> <label class=white-text for=room>Game Name</label><div ng-if=errorCollection.errors.room class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.room"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div><div class="input-field col s6"><input ng-model=Hangman.data.forms.create.player_limit id=player_limit type=tel class=validate> <label class=white-text for=player_limit>Player Limit</label><div ng-if=errorCollection.errors.player_limit class="card-panel card-error red"><ul><li ng-repeat="error in errorCollection.errors.player_limit"><i class="tiny mdi-alert-warning"></i> {{error}}</li></ul></div></div></div><button class="btn pink waves-effect" type=submit>Start It!</button></form></div></div>'),e.put("/cache/games/hangman/index.html",'<div ng-show="Hangman.data.games.length == 0" class="card-panel red">You haven\'t made any games yet you fool.</div><div class=row><div class="col s4"><h2>Chat</h2><div class=row><div class="col s12">do all the chats here :)</div></div></div><div class="col s8"><div class=row><div class="col s12 game-entry" ng-repeat="game in Hangman.data.games"><div class="card-panel light-blue lighten-3"><p>Game id: {{game.id}}</p><p>Room Name: {{game.room}}</p><p>Player Limit: {{game.player_limit}}</p><p>Owner: {{game.owner.full_name}}</p><p ng-repeat="player in game.players">{{$index+1}}: {{player.full_name}}</p><a ui-sref="app.master.hangman.lobby({id: game.id})">Enter</a></div></div></div></div></div><a ui-sref=app.master.hangman.create class="btn-fixed btn-floating btn-large waves-effect waves-light red"><i class="md mdi-action-favorite"></i></a>'),e.put("/cache/games/hangman/lobby.html",'<div class=row><div class="col s6"><div class="card-panel light-blue" ng-if=Hangman.data.game><h2>Current Players</h2><h4>Owner => {{Hangman.data.game.owner.full_name}}</h4><p ng-repeat="player in Hangman.data.game.players">{{player.full_name}}</p></div></div><div class="col s6"><div class="card-panel light-blue"></div></div></div>')}]);