...
!function(){"use strict";function e(e,t,o,n){o.baseUrl=n.baseUrl,o.loginUrl="/auth",e.state("app",{url:"","abstract":!0,template:"<div ui-view></div>",controller:"BaseCtrl as Base"}).state("app.master",{url:"",templateUrl:"/cache/layouts/master.html","abstract":!0}).state("app.simple",{url:"",templateUrl:"/cache/layouts/simple.html","abstract":!0}).state("app.master.chat",{url:"/chat",title:"Chat",controller:"ChatCtrl as Base",templateUrl:"/cache/chat.html"}).state("app.master.profile",{url:"/profile",title:"Profile",controller:"ProfileCtrl as Profile",templateUrl:"/cache/profile/index.html"}).state("app.master.profile-edit",{url:"/profile/edit",title:"Profile | Edit",controller:"ProfileCtrl as Profile",templateUrl:"/cache/profile/edit.html"}).state("app.simple.register",{url:"/register",title:"Register",controller:"RegisterCtrl as Register",templateUrl:"/cache/sessions/register.html"}),t.otherwise("/profile")}angular.module("orange.config",["orange.constants"]).config(e),e.$inject=["$stateProvider","$urlRouterProvider","$authProvider","Config"]}(),function(){"use strict";angular.module("orange.constants",[]).constant("Config",{baseUrl:"http://localhost:4200/",siteName:"Orange Gaming"})}(),function(){"use strict";angular.module("orange",["btford.socket-io","satellizer","http-auth-interceptor","ui.router","orange.templates","orange.config","orange.factories","orange.services","orange.controllers","orange.run"])}(),function(){"use strict";function e(e,t){$(document).ready(function(){$(".dropdown-button").dropdown()}),e.$on("$stateChangeSuccess",function(){e.title=t.current.title})}angular.module("orange.run",[]).run(e),e.$inject=["$rootScope","$state"]}(),function(){"use strict";function e(e,t,o,n,r,i){function a(){t.session=r.session.session,t.config=i,l.data={forms:{login:{username:"",password:""}}},console.log("Loading base controller"),r.profile()}function s(){$(function(){$("#login").openModal()})}function c(){$(function(){$("#login").closeModal()})}var l=this;a(),e.$on("event:auth-loginRequired",function(e,t){r.logout(),s()}),e.$on("event:auth-loginConfirmed",function(e,t){c()}),t.register=function(){c(),o.go("app.simple.register")},t.login=function(){r.login(l.data.forms.login).then(function(e){l.data.forms.login={},n.loginConfirmed(),c()},function(e){console.log(e)})},t.logout=function(){e.$broadcast("event:auth-loginRequired")}}angular.module("orange.controller.base",[]).controller("BaseCtrl",e),e.$inject=["$rootScope","$scope","$state","authService","SessionsFactory","Config"]}(),function(){"use strict";function e(e,t){function o(){console.log("Loading the chat controller")}o();var n,r=1e3;$("#m");$(document).ready(function(){function e(){t.emit("chat:typing-stopped")}$(document).keypress(function(t){13==t.which&&e()}),$("form").submit(function(){return t.emit("chat:message",$("#m").val()),$("#m").val(""),!1}),t.on("chat:message",function(e){$("#messages").append($("<li>").text(e))}),t.on("chat:typing",function(){$("#typing").text("A user is typing")}),t.on("chat:typing-stopped",function(){$("#typing").text("")}),$("#m").keyup(function(){clearTimeout(n),t.emit("chat:typing"),n=setTimeout(e,r)}),$("#m").keydown(function(){t.emit("chat:typing"),clearTimeout(n)})})}angular.module("orange.controller.chat",[]).controller("ChatCtrl",e),e.$inject=["$scope","SocketFactory"]}(),function(){"use strict";angular.module("orange.controllers",["orange.controller.base","orange.controller.chat","orange.controller.profile","orange.controller.register"])}(),function(){"use strict";function e(e){function t(){console.log("loaded the profile controller"),o.data={}}var o=this;t()}angular.module("orange.controller.profile",[]).controller("ProfileCtrl",e),e.$inject=["$scope"]}(),function(){"use strict";function e(e){function t(){o.data={}}var o=this;t()}angular.module("orange.controller.register",[]).controller("RegisterCtrl",e),e.$inject=["$scope"]}(),function(){"use strict";angular.module("orange.factories",["orange.factory.socket","orange.factory.sessions"])}(),function(){"use strict";function e(e,t,o,n,r,i,a,s){function c(e){var t=n.defer(),o={ignoreAuthModule:!0};return r.login(e,o).then(function(e){s.create(g,e.data.user),t.resolve()})["catch"](function(e){t.reject(e)}),t.promise}function l(){r.logout(),s.destroy(g)}function u(){var r=localStorage.getItem("current_user"),a=n.defer();return"null"===r||null===r||void 0===r||"undefined"===r?(t(function(){o.$broadcast("event:auth-loginRequired")},1),a.reject()):(r=JSON.parse(r),e.get(i.baseUrl+"api/users/"+r.id).success(function(e){s.create(g,e.data),a.resolve()}).error(function(e){a.reject(e)})),a.promise}var g={session:{current_user:null,logged_in:!1}},f={login:c,logout:l,profile:u,session:g};return f}angular.module("orange.factory.sessions",[]).factory("SessionsFactory",e),e.$inject=["$http","$timeout","$rootScope","$q","$auth","Config","JWTService","SessionsService"]}(),function(){"use strict";function e(e){var t=io.connect("http://localhost:4200"),o=e({ioSocket:t});return o}angular.module("orange.factory.socket",[]).factory("SocketFactory",e),e.$inject=["socketFactory"]}(),function(){"use strict";function e(e){var t=this;t.parse=function(t){var o=t.split(".")[1],n=o.replace("-","+").replace("_","/");return JSON.parse(e.atob(n))},t.save=function(t){e.localStorage._satellizer_token=t},t.get=function(){return e.localStorage.satellizer_token}}angular.module("orange.service.jwt",[]).service("JWTService",e),e.$inject=["$window"]}(),function(){"use strict";angular.module("orange.services",["orange.service.jwt","orange.service.sessions"])}(),function(){"use strict";function e(e){var t=this;t.create=function(t,o){e.localStorage.current_user=JSON.stringify(o),t.session.current_user=o,t.session.logged_in=!0},t.destroy=function(t){e.localStorage.removeItem("_satellizer_token"),e.localStorage.removeItem("satellizer_token"),e.localStorage.removeItem("current_user"),t.session.current_user={},t.session.logged_in=!1}}angular.module("orange.service.sessions",[]).service("SessionsService",e),e.$inject=["$window"]}();
angular.module("orange.templates",[]).run(["$templateCache",function(s){s.put("/cache/chat.html",'<ul id=messages></ul><div id=typing></div><div class=row><form class="col s12"><div class=row><div class="input-field col s12"><input id=m type=text class=validate> <label for=first_name>Message</label></div></div><button type=submit class="btn waves-effect">Send</button></form></div>'),s.put("/cache/games/index.html",""),s.put("/cache/includes/_nav.html",'<div class=navbar-fixed><nav><div class=nav-wrapper><a href=#! class=brand-logo>Logo</a><ul class="right hide-on-med-and-down"><li><a href=sass.html>Sass</a></li><li><a href=badges.html>Components</a></li></ul></div></nav></div>'),s.put("/cache/layouts/master.html",'<div class=navbar-fixed><ul id=dropdown1 class=dropdown-content><li><a ui-sref=app.master.profile>profile</a></li><li class=divider></li><li><a href="" ng-click=logout()>Logout</a></li></ul><nav><div class=nav-wrapper><a href=# class=brand-logo>{{config.siteName}}</a><ul id=nav-mobile class="right hide-on-med-and-down"><li ng-if=session.logged_in><a ui-sref=app.master.chat>Chat</a></li><li><a class=dropdown-button href=#! data-activates=dropdown1>{{session.current_user.name}}<i class="material-icons right"></i></a></li></ul></div></nav></div><div class=container><div class=row><div class="col s12" ui-view=""></div></div></div><div id=login class=modal><div class=modal-content><h4>Login</h4><div class=row><form class="col s12" ng-submit=login()><div class=row><div class="input-field col s12"><input ng-model=Base.data.forms.login.username id=username type=text class=validate> <label for=first_name>Username</label></div><div class="input-field col s12"><input id=last_name type=password ng-model=Base.data.forms.login.password class=validate> <label for=last_name>Password</label></div></div><button class="btn waves-effect" type=submit>Login</button></form></div><div class=row><div class="col s12"><p>Don\'t have an account? That\'s alright, register here :)</p><button class="btn pink waves-effect" ng-click=register()>Register</button></div></div></div></div>'),s.put("/cache/layouts/simple.html",'<div class=container><div class=row><div class="col s12" ui-view=""></div></div></div>'),s.put("/cache/profile/edit.html","edit me :)"),s.put("/cache/profile/index.html",'<div class=row><div class="col s8 offset-s2"><div class="card-panel teal lighten-1"><div class=section><span class=white-text><p><b>Name</b>: {{session.current_user.name}}</p><p><b>Username</b>: {{session.current_user.username}}</p><p><b>Email</b>: {{session.current_user.email}}</p></span></div><div class=divider></div><div class=section><button class="btn pink lighten-2 waves-effect" ui-sref=app.master.profile-edit>Edit</button></div></div></div></div>'),s.put("/cache/sessions/register.html",'<div class=row><form class="col s12"><div class=row><div class="input-field col s6"><input placeholder=Placeholder id=first_name type=text class=validate> <label for=first_name>First Name</label></div><div class="input-field col s6"><input id=last_name type=text class=validate> <label for=last_name>Last Name</label></div></div><div class=row><div class="input-field col s12"><input disabled="" value="I am not editable" id=disabled type=text class=validate> <label for=disabled>Disabled</label></div></div><div class=row><div class="input-field col s12"><input id=password type=password class=validate> <label for=password>Password</label></div></div><div class=row><div class="input-field col s12"><input id=email type=email class=validate> <label for=email>Email</label></div></div><button type=submit class="btn blue waves-effect">Get Started</button></form></div>')}]);